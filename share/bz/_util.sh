...
_title_from_pr () {
  ...
  local title=$(grep ^Title $d/pr | cut -d: -f 2- | sed -e 's,^ *,,' -e 's, *$,,')
...
_comment_from_pr () {
  ...
  local comment=$(head -$(($e-2)) $d/pr 2>/dev/null | tail -$(($e-$s-3)) 2>/dev/null)

  if [ -z "$comment" ]; then
    e=$(grep -n "^\[Comment #1\] " $d/pr | sed -e 's,:.*,,')
    comment=$(head -$(($e-2)) $d/pr | tail -$(($e-$s-3)))
    comment=$(echo "$comment" | sed -e '/^Created attachment/d' -e '/^.*\.diff$/d')
  fi
